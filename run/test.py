...
class Testcase06(unittest.TestCase):
    ...
    #由于一个用户只能绑定一个mec,暂时只能添加一个
    # @ddt.data(["10.1.3.1"], ["10.1.3.2"], ["10.1.3.3"])
    # @ddt.unpack
...
class Testcase08(unittest.TestCase):
    ...
    def testcase02(self):
        ...
        r=system().alter("usermame", "1122")
        self.assertIn(r,"编辑用户成功", "测试失败")

    def testcase03(self):
        '''验证用户创建后登陆成功[运维人员]'''
        r = system().login_yw("yunweitDAg2z")
...
if __name__ == '__main__':
    ...
    # suite.addTest(testcases)
    # BeautifulReport(testcases).report(filename=report_title, description='专网租户测试',report_dir=report_path,theme="theme_cyan")

    # #4.执行终端用户测试
    # testcases = unittest.TestLoader().loadTestsFromTestCase(Testcase04)
    # suite.addTest(testcases)
    # BeautifulReport(testcases).report(filename=report_title, description='终端用户测试', report_dir=report_path,theme="theme_cyan")

    #5.执行切片模版测试
    # testcases = unittest.TestLoader().loadTestsFromTestCase(Testcase05)
    # suite.addTest(testcases)
    # BeautifulReport(testcases).report(filename=report_title, description='切片模块测试', report_dir=report_path,theme="theme_cyan")

    # 6.执行MEC模块测试
    # testcases = unittest.TestLoader().loadTestsFromTestCase(Testcase06)
    # suite.addTest(testcases)
    # BeautifulReport(testcases).report(filename=report_title, description='MEC模块测试', report_dir=report_path,theme="theme_cyan")

    # 7.执行专网等级模块测试
    # testcases = unittest.TestLoader().loadTestsFromTestCase(Testcase07)
    # suite.addTest(testcases)
    # BeautifulReport(testcases).report(filename=report_title, description='专网等级模块测试',report_dir=report_path,theme="theme_cyan")


    # 8.执行系统用户管理测试
    # testcases = unittest.TestLoader().loadTestsFromTestCase(Testcase08)
    # suite.addTest(testcases)
    # BeautifulReport(testcases).report(filename=report_title, description='系统用户管理测试', report_dir=report_path,theme="theme_cyan")

    # 9.执行综合态势感知测试
    # testcases = unittest.TestLoader().loadTestsFromTestCase(Testcase09)
    # suite.addTest(testcases)
    # BeautifulReport(testcases).report(filename=report_title, description='综合态势感知测试', report_dir=report_path,theme="theme_cyan")
